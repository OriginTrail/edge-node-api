...
exports.defineProcessingPipelineId = async req => {
    ...
    const kmining_nq_pipeline_id = req.user.config.find(
        item => item.option === 'kmining_nq_pipeline_id'
    )?.value;
    ...
    if (
        req.file.mimetype === 'application/n-quads' ||
        req.file.originalname.endsWith('.nq')
    ) {
        return kmining_nq_pipeline_id;
    }
